...
!NetCDF variables
  ...
  integer :: ind, ind_u=-1, ind_v=-1, ind_w=-1, ind_qt=-1, ind_ql=-1, ind_thl=-1, ind_buoy=-1, ind_e12=-1, ind_p=-1, ind_sv(100)=-1
contains
!> Initializing fielddump. Read out the namelist, initializing the variables
  subroutine initfielddump
    ...
    use modstat_nc,only : lnetcdf,open_nc, define_nc,ncinfo,nctiminfo,writestat_dims_nc,lclassic_global=>lclassic
    implicit none
    integer :: ierr, n
    character(3) :: csvname
    ...
    if (lnetcdf) then
      ...
      call open_nc(trim(output_prefix)//fname,  ncid,nrec,n1=ceiling(1.0*imax/ncoarse),n2=ceiling(1.0*jmax/ncoarse),n3=khigh-klow+1,lclassic_par=lclassic)
      if (nrec==0) then
